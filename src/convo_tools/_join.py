...
def _parse_timestamp(ts: float | int | str | None) -> float | None:
    if ts is None:
        return None
...
def _ts_iso(ts_f: float | None) -> str:
    if ts_f is None:
        ts_f = time.time()
    return datetime.fromtimestamp(ts_f, tz=UTC).strftime(
        "%Y-%m-%dT%H:%M:%S.%fZ"
    )
...
def _content_hash(conv: dict[str, Any]) -> str:
...
def detect_format(data: dict[str, Any] | list[Any], filename: str) -> str:
    if filename.endswith(".md"):
        return "openai_md"
    if not isinstance(data, (dict, list)):
        return "unknown"
    if isinstance(data, dict):
        if "mapping" in data and (
            "conversation_id" in data or "id" in data
        ):
...
def _parse_anthropic(convs: list[Any]) -> list[dict[str, Any]]:
...
def _parse_openai(items: list[Any]) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for c in items:
        ...
        while stack:
            ...
            for child in _sorted_children(mapping, nid):
                if child not in visited:
                    stack.append(child)
        conv_id = (
            c.get("conversation_id") or c.get("id", _uid())
        )
        out.append(
            {
                "id": conv_id,
...
def _parse_message_array(
    data: list[Any], source_format: str
) -> list[dict[str, Any]]:
...
def _parse_openai_md(
    ...
    for line in lines:
        ...
        if tm:
            ...
            continue
        rm = re.match(
            r"^####\s+(You|ChatGPT|Human|Assistant|User):\s*$",
            line,
            re.IGNORECASE,
        )
        if rm:
            ...
            cur_role = role_map.get(
                rm.group(1).lower(), "user"
            )
            cur_lines = []
            continue
        if cur_role:
    ...
    if not title and msgs:
        title = (
            msgs[0]["content"][:80] if msgs[0]["content"] else ""
        )
    return [
        {
            "id": _uid(source_file),
...
PARSER_DISPATCH: dict[str, Callable[..., list[dict[str, Any]]]] = {
    ...
    "singles_claude": lambda d: _parse_message_array(
        d, "singles_claude"
    ),
    "generic_message_array": lambda d: _parse_message_array(
        d, "generic"
    ),
}
...
def parse_file(filepath: str) -> list[dict[str, Any]]:
    ...
    if not parser:
        print(
            f"  Skipping {filepath}: unknown format ({fmt})",
            file=sys.stderr,
        )
        return []
    convs = parser(data)
        ...
        for i, m in enumerate(conv["messages"]):
            ...
            mapping[mid] = {
                ...
                "message": {
                    ...
                    "weight": 1.0,
                    "metadata": {
                        "model_slug": m.get("model") or "auto"
                    },
                    "recipient": "all",
                },
                "parent": prev_id,
...
def run_join(args: argparse.Namespace) -> None:
    ...
    print(
        f"Found {len(files)} file(s). Processing...", file=sys.stderr
    )
    ...
    print(
        f"\nTotal: {len(all_convs)} conversation(s) from {len(files)} file(s).",
        file=sys.stderr,
    )
    ...
    if args.dedup:
        ...
        if dup_count:
            print(
                f"Dedup: removed {dup_count} duplicate(s), {len(unique)} unique remain.",
                file=sys.stderr,
            )
        all_convs = unique

    writers = {
        "anthropic": write_anthropic,
        "openai": write_openai,
        "gemini": write_gemini,
    }
    ...
    if args.output:
        with open(args.output, "w", encoding="utf-8") as f:
            f.write(blob)
        print(
            f"Wrote {len(out)} items to {args.output}",
            file=sys.stderr,
        )
    else:
